...
def get_secret(repo_url):
    base_url = repo_url

    headers = {
        'Authorization': 'Bearer <insert-github-token>'
    }
    ...
    if response.status_code == 200:
        ...
        data_decodificada = base64.b64decode(data['content']).decode('utf-8')
        pattern = r'sk-[A-Za-z0-9]+'
        global secrets
        secrets = secrets + re.findall(pattern, data_decodificada)
...
for combinacion in combinaciones:
    ...
    with open('secrets.txt', 'a') as file:
        unique_secrets_list = list(set(secrets))
        file.writelines("%s\n" % item for item in unique_secrets_list)
